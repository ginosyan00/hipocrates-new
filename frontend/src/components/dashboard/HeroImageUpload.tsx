...
interface HeroImageUploadProps {
  ...
  onUpload: (heroImage: string) => Promise<void>;
  isLoading?: boolean;
}
...
export const HeroImageUpload: React.FC<HeroImageUploadProps> = ({ 
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Проверка размера (макс 10MB для больших изображений)
    ...
    <div className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-text-100 mb-2">
          Главное изображение клиники
        </label>
        <p className="text-xs text-text-10 mb-4">
          Это изображение будет отображаться на главной странице вашей клиники для пациентов.
          Рекомендуемый размер: 1920x600px или больше. Форматы: JPG, PNG, GIF.
        </p>
      </div>
          ...
          <div className="w-full max-w-md h-48 border-2 border-stroke rounded-lg overflow-hidden bg-bg-primary flex items-center justify-center">
            {preview ? (
              ...
              <div className="text-text-10 text-sm text-center p-4">
                <svg
                  className="w-16 h-16 mx-auto mb-2 text-text-20"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                <p className="text-xs text-text-20 mt-1">Рекомендуемый размер: 1920x600px</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex-1 space-y-3">
          <div>
            ...
            <p className="text-xs text-text-10 mt-2">
              Форматы: JPG, PNG, GIF. Максимальный размер: 10MB
            </p>
          </div>
